...
def sbc_body(country_en, country_fa, fee_total, inst, embassy_note=True, companions_en=None, companions_fa=None):
    ...
    rows = [
      ...
      (H("5. Payment Terms and Obligations") + P(f"5.1 The Professional Fees: {fee_total} Euro to be paid as follows:") +
       (inst[0] if isinstance(inst, tuple) else UL([f"{a} Euro {w}" for a, w, _, _ in inst] + (["The costs related to the embassy and the fee application are the responsibility of the applicant."] if embassy_note else []))) +
       ...
       (inst[1] if isinstance(inst, tuple) else UL([f"مبلغ {aw} یورو {wf}" for _, _, aw, wf in inst] + (["هزینه های مربوط به سفارت و اپلیکیشن فی بر عهده متقاضی است."] if embassy_note else []))) +
