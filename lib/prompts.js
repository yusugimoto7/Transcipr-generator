...
// Turn REAL, freshly-fetched news articles into Farsi topic cards. Each article
// is given an [id]; the model writes the hook and returns that id, so the
// server can re-attach the real source_url + real published date (the model
// never invents them). This replaces LLM "web search" with grounded rewriting.
export function topicsFromNewsPrompt(items, todayStr, excludeTitles = []) {
  const list = items
    .map(
      (it, i) =>
        `[${i}] (${it.source_name}${it.published ? ", " + it.published.slice(0, 10) : ""}) ${it.title}\n${it.snippet || ""}`
    )
    .join("\n\n");
  const excludeBlock = excludeTitles.length
    ? `\n\nAVOID REPEATS — skip anything that overlaps these already-used titles:\n${excludeTitles
        .map((t) => `- ${t}`)
        .join("\n")}`
    : "";

  return `You are the content strategist for Sugimoto Visa, a Persian-language Canadian & European immigration brand (Instagram @sugimotovisa). Audience: Persian-speaking (Iranian) immigrants and aspirants. TODAY IS ${todayStr}.

Below are REAL, recent immigration news articles (already fetched from trusted feeds, each with an [id]). Turn the RELEVANT ones into short-video topic cards.

SELECT: keep only items genuinely useful to an Iranian audience aiming to immigrate to/stay in Canada or Europe — policy/rule changes, PNP (especially BC PNP), study permits & PGWP, work permits & LMIA, bans/caps/deadlines, processing-time and fee changes, notable Federal Court decisions, and Europe (Germany Opportunity Card, Portugal, etc.).
SKIP: Express Entry DRAW results / CRS cut-offs / "latest draw" round-ups (the brand covers these elsewhere), US immigration, pure opinion/listicles, sponsored content, and anything off-topic.
ALSO SKIP (critical): content-free items that have no substance to build a real topic on — media advisories, "minister to make an announcement", readouts, photo ops, event notices. If the article text does not contain an actual policy, rule, program, or decision, DROP it.

⛔️ ANTI-INVENTION RULE: the title_fa / title_en / why_now you write must be strictly supported by that specific article's title and snippet. NEVER invent a different topic, program, number, or claim that the article doesn't actually say. If a snippet is too thin to tell what the real news is, DROP that item rather than guessing. It is far better to return 3 accurate cards than 8 with any fabricated one.

For each KEPT item, write a card in the brand's voice: punchy, high-stakes/anxiety framing where honest, name "Iranians" directly when it fits, specific not generic — but ONLY using what the article actually says. Keep the best (up to 12; fewer is fine). RECENCY FIRST: when choosing which to keep, the NEWEST items win — prefer today's and yesterday's news over anything older.

VARIETY RULE: do NOT return several cards on the same underlying development. If three articles cover one announcement, write ONE card for it and spend the remaining slots on different beats. Aim to spread the cards across different fields (PNP, Study, Work Permit, Policy, Court, Europe) rather than returning five cards from whichever beat happens to be loudest this week.

ARTICLES:
${list}
${excludeBlock}

Return ONLY a JSON array (no markdown, no preamble) of objects, one per KEPT article, each with:
- "id": the integer [id] of the source article (REQUIRED, copy it exactly)
- "title_fa": the Farsi hook/title, ready to put on screen
- "title_en": short English title
- "field": one of "PNP","Study","Work Permit","LMIA","Policy","Court","Europe" (never "Express Entry")
- "page": "CA" or "EU"
- "why_now": ONE Persian sentence tying it to this specific development
- "score": integer 60-98 engagement-potential score
Order by "score" descending. Output the JSON array and nothing else.`;
}
...
// English Reel prompt for the English-speaking market (the "en" script above
// is a companion to the Farsi video and still frames the story for Iranians).
// Same grounding contract. Output is a short on-screen title plus the spoken
// script, sized for a 45-75s Reel so it can go straight to a voice + avatar
// render. The hook names who is affected per the source, never a nationality,
// so it works whether the channel targets newcomers in Canada or overseas.
export function reelPromptEn(topic, sourceText = "") {
  const source = String(sourceText || topic.snippet || "").trim();
  const sourceBlock =
    source.length > 60
      ? source.slice(0, 4500)
      : "(Full article text is unavailable. Write only from this title and short description and add nothing of your own.)";

  return `You write short vertical-video scripts (Instagram Reels, TikTok, YouTube Shorts) for Sugimoto Visa, a licensed Canadian immigration firm, for an English-speaking international audience: people planning to move to Canada and newcomers already there. The presenter is a licensed consultant speaking to camera. Accuracy is critical.

GOLDEN RULE (non-negotiable): cover ONLY the specific news in the SOURCE TEXT and use ONLY facts that appear there. Never invent a number, date, program name, score, fee, deadline or eligibility rule. If the source is thin, write a shorter truthful script. If it is only an advisory ("details will be announced"), say exactly that.

Speak the facts directly in your own voice. Never say "according to the source", "the article says" or "as reported".
...
Write it like this:
1. The first spoken sentence is the hook: under 15 words, names who this affects (for example international students, work permit holders, spouses, skilled workers) as the source supports. Never name a nationality unless the source is specifically about that nationality.
2. What changed, when, and who it applies to, in plain English a non-native speaker follows easily. Short sentences. No jargon without a four-word explanation.
3. What it means for the viewer and one realistic next step (check eligibility, wait for official details, prepare a document).
4. Close with one soft line inviting them to book a consultation with Sugimoto Visa. No promises of approval, no "guaranteed", no urgency pressure.

Length: 110 to 170 spoken words, fewer if the source is thin.

Output format, exactly:
TITLE: <on-screen title, 3 to 7 words, no emoji>

<spoken script as plain paragraphs, no labels, no hashtags, no stage directions>`;
}
