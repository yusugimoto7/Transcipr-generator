...
export const CORE_FEEDS = [
  ...
  // IRCC Ministerial Instructions and the notices index on canada.ca:
  // www.canada.ca was unreachable from this environment (TLS handshake
  // completes but no HTTP response ever arrives), so RSS/Atom availability
  // on those specific pages is unverified, not confirmed absent — retry
  // when this can be checked from a network that reaches canada.ca. The
  // "IRCC" entry above (api.io.canada.ca news centre) is the closest real
  // feed IRCC publishes today.
