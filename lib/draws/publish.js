...
export async function postTelegram(item) {
  ...
  const chatId = process.env.DRAWS_TELEGRAM_CHANNEL_ID || process.env.TELEGRAM_CHANNEL_ID;
  if (!token || !chatId) return skip("TELEGRAM_BOT_TOKEN / TELEGRAM_CHANNEL_ID not set");
