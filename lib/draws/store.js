...
async function fetchKeys() {
  // Apps Script answers a GET by redirecting to a script.googleusercontent.com
  // echo URL, and those are cacheable. A read served from cache straight after a
  // write looks identical to a write that never happened — which is exactly the
  // failure this store exists to catch, so the read must never be cached.
...
export async function markPosted(item, results = {}) {
  ...
  // The Sheet refuses to store a key twice and reports how many it actually
  // added. added === 0 means the key was already there before this call — so
  // another run claimed this draw between our read and our write, and it is
  // publishing it. Ours must not publish it too.
  return {
    ok: true,
    verified: true,
    alreadyClaimed: typeof wrote?.added === "number" && wrote.added === 0,
  };
}
...
function explainDeadWrite(wrote) {
  ...
  if (wrote?.added > 0) {
    return "The Apps Script says it appended the row, but reading it straight back does not show it. That is a stale read rather than a failed write — check the PostedDraws tab: if the row is there, the read-back is being served from cache.";
  }
