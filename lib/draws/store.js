...
//
// 1. A POST to an Apps Script /exec URL answers with a 302. Per the fetch spec a
//    followed 302 turns POST into GET, which DROPS THE BODY — the write silently
//    never happens and the call still looks successful. So redirects are followed
//    by hand, re-issuing the POST.
//
// 2. Never trust the write. After recording, the key is read back and confirmed.
//    An unverified write is reported as a failure, because a write that silently
//    fails is what let the same draw post every 15 minutes.
...
// POST that survives Apps Script's 302 (see rule 1 above).
async function postJson(url, body, hops = 0) {
  ...
  if ([301, 302, 303, 307, 308].includes(res.status) && hops < 5) {
    ...
    if (!next) throw new Error(`redirect ${res.status} without a location`);
    return postJson(new URL(next, url).toString(), body, hops + 1);
  }
...
export async function markPosted(item, results = {}) {
  ...
  const wrote = await postJson(sheetUrl(), {
    secret: sheetSecret(),
    library: [record],
  });
