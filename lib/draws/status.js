...
export function setupStatus() {
  ...
  const dedup = {
    ...
    ready: storeIsDurable(),
    missing: ["SHEET_WEBHOOK_URL", "SHEET_SECRET"].filter((v) => !has(v)),
    note: storeIsDurable()
      ? "Confirm it actually reads AND writes with /api/draws/selftest?key=… — a store that reads but does not write looks fine here and still lets a draw repeat."
