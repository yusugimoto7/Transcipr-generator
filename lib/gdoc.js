...
export async function createGoogleDoc({ title, html }) {
  ...
  // An old Apps Script (without the "doc" action) silently returns {ok:true}
  // with no URL. Treat a missing URL as a clear, actionable error.
  if (!data.url) {
    throw new Error(
      "اسکریپت گوگل هنوز آپدیت نشده. فایل google-apps-script.gs رو دوباره پیست کن، سرویس Drive API رو فعال کن (Services +) و وب‌اپ رو Re-deploy کن."
    );
  }
